import { useEffect, useRef } from "react"

// ─── CSS ───────────────────────────────────────────────────────────────────────
const GLOBAL_CSS = `
@keyframes scrapbook-scroll {
  from { transform: translateX(0); }
  to   { transform: translateX(-33.3334%); }
}
@keyframes scrapbook-scroll-rev {
  from { transform: translateX(-33.3334%); }
  to   { transform: translateX(0); }
}
@keyframes sb-ticker {
  from { transform: translateX(0); }
  to   { transform: translateX(-50%); }
}
@keyframes bracket-tl {
  from { width: 0; height: 0; opacity: 0; }
  to   { width: 28px; height: 28px; opacity: 1; }
}
.sb-strip     { animation: scrapbook-scroll 28s linear infinite; }
.sb-strip-rev { animation: scrapbook-scroll-rev 34s linear infinite; }
.sb-strip:hover, .sb-strip-rev:hover { animation-play-state: paused; }
.sb-ticker    { animation: sb-ticker 25s linear infinite; }
.sb-bracket   { animation: bracket-tl 0.4s cubic-bezier(0.22,1,0.36,1) forwards; }
`

// ─── Data ──────────────────────────────────────────────────────────────────────
const PHOTOS = [
  { src: "./photos/cruise_selfie.jpg",     caption: "Island vibes — cruise excursion stop",  idx: "01" },
  { src: "./photos/espresso_martinis.jpg", caption: "Espresso martinis on the ship",          idx: "02" },
  { src: "./photos/beach_bar.jpg",         caption: "Beach bar afternoon",                    idx: "03" },
  { src: "./photos/resort_pose.jpg",       caption: "Golden hour at the resort",              idx: "04" },
  { src: "./photos/toy_shopping.jpg",      caption: "Little shopper",                         idx: "05" },
  { src: "./photos/playground.jpg",        caption: "She owns every room she walks into",     idx: "06" },
]

const TICKER_TEXT = "ISLAND VIBES · CRUISE LIFE · BEACH BAR · GOLDEN HOUR · LITTLE SHOPPER · OUR GIRL · "

const P = {
  cream:      "#F5EFE6",
  sand:       "#D9C5A0",
  terracotta: "#C1673A",
  espresso:   "#2C1A0E",
  dark:       "#1A0F07",
  mid:        "#3D2314",
}

const PH_GRADIENTS = [
  `135deg, #C1673A 0%, #D9C5A0 100%`,
  `135deg, #7A3219 0%, #C1673A 100%`,
  `135deg, #D9C5A0 0%, #C1673A 80%`,
  `135deg, #C1673A 20%, #2C1A0E 100%`,
  `135deg, #3D2314 0%, #C1673A 65%`,
  `135deg, #C1673A 0%, #F5EFE6 100%`,
]

const SECTION_BG = [P.espresso, P.mid, "#231208", P.espresso, P.mid, "#231208"]

// ─── Utility ───────────────────────────────────────────────────────────────────
function loadScript(src) {
  return new Promise((resolve, reject) => {
    if (document.querySelector(`script[src="${src}"]`)) { resolve(); return }
    const s = document.createElement("script")
    s.src = src; s.onload = resolve; s.onerror = reject
    document.head.appendChild(s)
  })
}

// ─── BannerCard ────────────────────────────────────────────────────────────────
function BannerCard({ photo }) {
  return (
    <div style={{
      flexShrink: 0, width: "clamp(160px,18vw,240px)", height: "clamp(200px,22vw,290px)",
      borderRadius: "6px", overflow: "hidden", position: "relative",
      boxShadow: "0 8px 32px rgba(0,0,0,0.55),0 0 0 1px rgba(217,197,160,0.07)",
    }}>
      <img src={photo.src} alt={photo.caption}
        onError={e => { e.currentTarget.style.display="none"; const p=e.currentTarget.parentElement?.querySelector("[data-phb]"); if(p) p.style.display="flex" }}
        style={{ width:"100%",height:"100%",objectFit:"cover",display:"block" }}
      />
      <div data-phb="true" style={{
        display:"none",position:"absolute",inset:0,
        background:`linear-gradient(135deg,${P.terracotta} 0%,${P.espresso} 100%)`,
        alignItems:"center",justifyContent:"center",
      }}>
        <span style={{ fontFamily:"'DM Mono',monospace",color:"rgba(245,239,230,0.25)",fontSize:"0.6rem",letterSpacing:"0.3em" }}>{photo.idx}</span>
      </div>
      <div style={{
        position:"absolute",inset:0,
        background:"linear-gradient(to top,rgba(26,15,7,0.8) 0%,transparent 55%)",
        display:"flex",alignItems:"flex-end",padding:"0.75rem",pointerEvents:"none",
      }}>
        <p style={{ fontFamily:"'Cormorant Galatia',serif",fontSize:"0.75rem",fontStyle:"italic",color:P.sand,margin:0,lineHeight:1.3,opacity:0.85 }}>{photo.caption}</p>
      </div>
    </div>
  )
}

// ─── Component ─────────────────────────────────────────────────────────────────
export default function ScrapbookPage() {
  const progressBarRef       = useRef(null)
  const bannerRef            = useRef(null)
  const bannerHeadRef        = useRef(null)
  const videoSectionRef      = useRef(null)
  const videoRef             = useRef(null)
  const videoProgressLineRef = useRef(null)
  const counterNumRef        = useRef(null)

  // per-section refs
  const sectionRefs  = useRef([])   // <section> elements
  const imgPanelRefs = useRef([])   // photo half panel (clip-path target)
  const imgRefs      = useRef([])   // <img> inside (parallax)
  const bigNumRefs   = useRef([])   // huge number display
  const eyebrowRefs  = useRef([])   // small label above number
  const wordRefs     = useRef([])   // word spans for masked reveal (2-D array)
  const sweepRefs    = useRef([])   // growing terracotta rule

  useEffect(() => {
    if (!document.getElementById("sb-global-css")) {
      const s = document.createElement("style")
      s.id = "sb-global-css"; s.textContent = GLOBAL_CSS
      document.head.appendChild(s)
    }

    const fontLink = document.createElement("link")
    fontLink.rel = "stylesheet"
    fontLink.href = "https://fonts.googleapis.com/css2?family=Cormorant+Galatia:ital,wght@0,300;0,400;0,600;1,300;1,400&family=DM+Sans:opsz,wght@9..40,300;9..40,400;9..40,500&family=DM+Mono:wght@300;400&display=swap"
    if (!document.querySelector(`link[href="${fontLink.href}"]`)) document.head.appendChild(fontLink)

    let killAll = () => {}

    loadScript("https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.2/gsap.min.js")
      .then(() => loadScript("https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.2/ScrollTrigger.min.js"))
      .then(() => {
        const { gsap } = window
        const { ScrollTrigger } = window
        if (!gsap || !ScrollTrigger) return
        gsap.registerPlugin(ScrollTrigger)

        // ── 0. Video scrub (instant seek — no rAF buffer) ──────────────────
        const vid = videoRef.current
        if (vid && videoSectionRef.current) {
          vid.pause(); vid.currentTime = 0; vid.preload = "auto"

          const setupScrub = () => {
            ScrollTrigger.create({
              trigger: videoSectionRef.current,
              start: "top top", end: "+=300%",
              pin: true, pinSpacing: true,
              scrub: true,   // instant: 0 lag, follows scroll 1-to-1
              onUpdate: self => { if (vid.duration) vid.currentTime = self.progress * vid.duration },
            })
            if (videoProgressLineRef.current) {
              gsap.fromTo(videoProgressLineRef.current, { scaleX: 0 }, {
                scaleX: 1, ease: "none",
                scrollTrigger: { trigger: videoSectionRef.current, start: "top top", end: "+=300%", scrub: true },
              })
            }
          }

          if (vid.readyState >= 1) setupScrub()
          else vid.addEventListener("loadedmetadata", setupScrub, { once: true })
        }

        // ── 1. Page progress bar ─────────────────────────────────────────────
        if (progressBarRef.current) {
          gsap.to(progressBarRef.current, {
            scaleX: 1, ease: "none",
            scrollTrigger: { trigger: document.body, start: "top top", end: "bottom bottom", scrub: 0.2 },
          })
        }

        // ── 2. Banner heading ────────────────────────────────────────────────
        if (bannerHeadRef.current) {
          gsap.fromTo(bannerHeadRef.current,
            { opacity: 0, y: 50 },
            { opacity: 1, y: 0, ease: "power3.out",
              scrollTrigger: { trigger: bannerHeadRef.current, start: "top 90%", end: "top 50%", scrub: 1 } }
          )
        }

        // ── 3. 3D banner strips ──────────────────────────────────────────────
        if (bannerRef.current) {
          gsap.fromTo(bannerRef.current,
            { rotateX: 32, opacity: 0, y: 70 },
            { rotateX: 0, opacity: 1, y: 0, ease: "power3.out",
              scrollTrigger: { trigger: bannerRef.current, start: "top 85%", end: "top 28%", scrub: 1.2 } }
          )
        }

        // ── 4. Full-bleed photo sections ─────────────────────────────────────
        sectionRefs.current.forEach((section, i) => {
          if (!section) return
          const isLeft = i % 2 === 0

          // — clip-path wipe on the image panel —
          const imgPanel = imgPanelRefs.current[i]
          if (imgPanel) {
            const clipStart = isLeft ? "inset(0 100% 0 0)" : "inset(0 0 0 100%)"
            gsap.fromTo(imgPanel,
              { clipPath: clipStart },
              { clipPath: "inset(0 0% 0 0%)", ease: "power2.inOut",
                scrollTrigger: { trigger: section, start: "top 80%", end: "top 15%", scrub: 1 } }
            )
          }

          // — parallax on inner image —
          const img = imgRefs.current[i]
          if (img) {
            gsap.fromTo(img,
              { yPercent: -10 },
              { yPercent: 10, ease: "none",
                scrollTrigger: { trigger: section, start: "top bottom", end: "bottom top", scrub: true } }
            )
          }

          // — eyebrow label slides in —
          const eyebrow = eyebrowRefs.current[i]
          if (eyebrow) {
            gsap.fromTo(eyebrow,
              { x: isLeft ? -40 : 40, opacity: 0 },
              { x: 0, opacity: 1, ease: "power3.out",
                scrollTrigger: { trigger: section, start: "top 78%", end: "top 40%", scrub: 1 } }
            )
          }

          // — big number: scale + opacity —
          const bigNum = bigNumRefs.current[i]
          if (bigNum) {
            gsap.fromTo(bigNum,
              { scale: 1.35, opacity: 0, y: 30 },
              { scale: 1, opacity: 1, y: 0, ease: "power3.out",
                scrollTrigger: { trigger: section, start: "top 75%", end: "top 30%", scrub: 1 } }
            )

            // Scramble effect fires once on enter
            ScrollTrigger.create({
              trigger: section, start: "top 65%", once: true,
              onEnter: () => {
                const target = PHOTOS[i].idx
                let count = 0
                const digits = "0123456789"
                const iv = setInterval(() => {
                  if (count++ >= 14) { clearInterval(iv); bigNum.textContent = target; return }
                  bigNum.textContent =
                    digits[Math.floor(Math.random() * digits.length)] +
                    digits[Math.floor(Math.random() * digits.length)]
                }, 35)
              },
            })
          }

          // — word-masked caption reveal —
          const words = wordRefs.current[i] || []
          if (words.length) {
            gsap.fromTo(words,
              { yPercent: 110, opacity: 0 },
              { yPercent: 0, opacity: 1, ease: "power4.out", stagger: 0.05,
                scrollTrigger: { trigger: section, start: "top 65%", end: "top 20%", scrub: 1 } }
            )
          }

          // — terracotta sweep rule —
          const sweep = sweepRefs.current[i]
          if (sweep) {
            gsap.fromTo(sweep,
              { scaleX: 0 },
              { scaleX: 1, ease: "power2.inOut",
                scrollTrigger: { trigger: section, start: "top 55%", end: "top 15%", scrub: 1 } }
            )
          }

          // — fixed counter update —
          if (counterNumRef.current) {
            ScrollTrigger.create({
              trigger: section, start: "top 55%",
              onEnter: () => flashCounter(i),
              onEnterBack: () => flashCounter(i),
            })
          }
        })

        function flashCounter(i) {
          const el = counterNumRef.current
          if (!el) return
          gsap.to(el, { opacity: 0, duration: 0.1, onComplete: () => {
            el.textContent = PHOTOS[i].idx
            gsap.to(el, { opacity: 1, duration: 0.1 })
          }})
        }

        killAll = () => ScrollTrigger.getAll().forEach(t => t.kill())
      })
      .catch(err => console.error("[Scrapbook] GSAP load failed:", err))

    return () => killAll()
  }, [])

  const handleImgError = (e) => {
    e.currentTarget.style.display = "none"
    const ph = e.currentTarget.parentElement?.querySelector("[data-placeholder]")
    if (ph) ph.style.display = "flex"
  }

  return (
    <div style={{ fontFamily: "'DM Sans',sans-serif", background: P.dark, overflowX: "hidden", color: P.cream }}>

      {/* ── Global progress bar ── */}
      <div aria-hidden="true" style={{ position:"fixed",top:0,left:0,right:0,height:"2px",background:"rgba(0,0,0,0.2)",zIndex:9999 }}>
        <div ref={progressBarRef} style={{
          height:"100%", background:`linear-gradient(90deg,${P.terracotta} 0%,${P.sand} 100%)`,
          transformOrigin:"left center",transform:"scaleX(0)",willChange:"transform",
        }} />
      </div>

      {/* ── Fixed photo counter ── */}
      <div aria-hidden="true" style={{
        position:"fixed",bottom:"2rem",right:"2.25rem",zIndex:200,
        display:"flex",alignItems:"center",gap:"0.5rem",
        fontFamily:"'DM Mono',monospace",fontSize:"0.65rem",letterSpacing:"0.08em",
        color:P.sand,opacity:0.55,pointerEvents:"none",
      }}>
        <span ref={counterNumRef} style={{ color:P.terracotta }}>01</span>
        <span style={{ opacity:0.35 }}>───</span>
        <span>{String(PHOTOS.length).padStart(2,"0")}</span>
      </div>

      {/* ╔══════════════════════════════════════════════════════╗
          ║  VIDEO SCRUB                                         ║
          ╚══════════════════════════════════════════════════════╝ */}
      <section ref={videoSectionRef} style={{ height:"100vh",position:"relative",overflow:"hidden",background:"#000" }}>
        <video ref={videoRef} src="./video/scene_keyframed.mp4" muted playsInline preload="auto"
          style={{ position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",display:"block" }}
        />

        {/* Film grain */}
        <div aria-hidden="true" style={{
          position:"absolute",inset:0,
          backgroundImage:`url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.85' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='0.04'/%3E%3C/svg%3E")`,
          backgroundSize:"200px 200px",mixBlendMode:"overlay",pointerEvents:"none",opacity:0.6,
        }} />

        {/* Scanlines */}
        <div aria-hidden="true" style={{
          position:"absolute",inset:0,
          backgroundImage:"repeating-linear-gradient(0deg,transparent,transparent 3px,rgba(0,0,0,0.025) 3px,rgba(0,0,0,0.025) 4px)",
          pointerEvents:"none",
        }} />

        <div aria-hidden="true" style={{
          position:"absolute",bottom:0,left:0,right:0,height:"45%",
          background:"linear-gradient(to bottom,transparent,#000)",pointerEvents:"none",
        }} />

        {/* Top-right HUD */}
        <div style={{
          position:"absolute",top:"2.25rem",right:"2.5rem",
          fontFamily:"'DM Mono',monospace",fontSize:"0.58rem",
          color:"rgba(245,239,230,0.28)",letterSpacing:"0.15em",
          pointerEvents:"none",textAlign:"right",lineHeight:2,
        }}>
          <div>2024</div>
          <div style={{ color:"rgba(193,103,58,0.5)" }}>● REC</div>
        </div>

        {/* Title */}
        <div style={{
          position:"absolute",
          bottom:"clamp(3rem,8vh,6rem)",left:"clamp(2rem,6vw,5rem)",
          pointerEvents:"none",
        }}>
          <p style={{
            fontFamily:"'DM Mono',monospace",color:"rgba(245,239,230,0.4)",
            fontSize:"0.58rem",letterSpacing:"0.45em",textTransform:"uppercase",margin:"0 0 1rem",
          }}>A · family · story</p>
          <h1 style={{
            fontFamily:"'Cormorant Galatia',serif",
            fontSize:"clamp(4rem,12vw,11rem)",fontWeight:300,color:"#fff",
            textTransform:"uppercase",letterSpacing:"0.06em",margin:0,lineHeight:0.9,
            textShadow:"0 4px 80px rgba(0,0,0,0.7)",
          }}>Our Story</h1>
          <div style={{ marginTop:"1.25rem",display:"flex",alignItems:"center",gap:"0.75rem" }}>
            <div style={{ width:"32px",height:"1px",background:P.terracotta,opacity:0.7 }} />
            <span style={{ fontFamily:"'DM Mono',monospace",fontSize:"0.57rem",color:"rgba(245,239,230,0.3)",letterSpacing:"0.3em" }}>
              SCROLL TO EXPLORE
            </span>
          </div>
        </div>

        {/* Video progress bar */}
        <div ref={videoProgressLineRef} style={{
          position:"absolute",top:0,left:0,height:"3px",width:"100%",
          background:P.terracotta,transformOrigin:"left center",transform:"scaleX(0)",
          willChange:"transform",pointerEvents:"none",opacity:0.8,
        }} />
      </section>

      {/* ╔══════════════════════════════════════════════════════╗
          ║  MARQUEE                                             ║
          ╚══════════════════════════════════════════════════════╝ */}
      <div style={{
        background:P.terracotta,overflow:"hidden",height:"2.8rem",
        display:"flex",alignItems:"center",
      }}>
        <div className="sb-ticker" style={{ display:"flex",whiteSpace:"nowrap",width:"200%" }}>
          {[TICKER_TEXT, TICKER_TEXT].map((t, ki) => (
            <span key={ki} style={{
              fontFamily:"'DM Mono',monospace",fontSize:"0.62rem",
              letterSpacing:"0.35em",color:"rgba(245,239,230,0.9)",
              textTransform:"uppercase",flex:"0 0 50%",
            }}>{t}{t}</span>
          ))}
        </div>
      </div>

      {/* ╔══════════════════════════════════════════════════════╗
          ║  3D BANNER                                           ║
          ╚══════════════════════════════════════════════════════╝ */}
      <section style={{ padding:"6rem 0 7rem",background:P.dark,overflow:"hidden",position:"relative" }}>
        <div ref={bannerHeadRef} style={{
          display:"flex",alignItems:"flex-end",justifyContent:"space-between",
          padding:"0 clamp(2rem,7vw,6rem)",marginBottom:"4rem",
          opacity:0,willChange:"transform,opacity",
        }}>
          <div>
            <p style={{
              fontFamily:"'DM Mono',monospace",fontSize:"0.58rem",
              color:P.terracotta,letterSpacing:"0.35em",textTransform:"uppercase",margin:"0 0 0.6rem",
            }}>The moments</p>
            <h2 style={{
              fontFamily:"'Cormorant Galatia',serif",
              fontSize:"clamp(2.5rem,6vw,5rem)",fontWeight:300,
              color:P.cream,margin:0,lineHeight:0.9,letterSpacing:"-0.02em",
            }}>Every Frame</h2>
          </div>
          <span style={{
            fontFamily:"'DM Mono',monospace",fontSize:"0.6rem",
            color:P.sand,opacity:0.25,letterSpacing:"0.2em",paddingBottom:"0.2rem",
          }}>06 photos</span>
        </div>

        <div ref={bannerRef} style={{ perspective:"900px",perspectiveOrigin:"50% 50%",willChange:"transform,opacity" }}>
          <div style={{ overflow:"hidden",marginBottom:"14px" }}>
            <div className="sb-strip" style={{ display:"flex",gap:"14px",width:"300%",transform:"rotateX(8deg)",transformOrigin:"50% 0%" }}>
              {[...PHOTOS,...PHOTOS,...PHOTOS].map((p,i) => <BannerCard key={`a-${i}`} photo={p} />)}
            </div>
          </div>
          <div style={{ overflow:"hidden" }}>
            <div className="sb-strip-rev" style={{ display:"flex",gap:"14px",width:"300%",transform:"rotateX(-6deg)",transformOrigin:"50% 100%" }}>
              {[...[...PHOTOS].reverse(),...[...PHOTOS].reverse(),...[...PHOTOS].reverse()].map((p,i) => (
                <BannerCard key={`b-${i}`} photo={p} />
              ))}
            </div>
          </div>
        </div>

        <div aria-hidden="true" style={{
          position:"absolute",inset:0,
          background:`linear-gradient(to right,${P.dark} 0%,transparent 8%,transparent 92%,${P.dark} 100%)`,
          pointerEvents:"none",zIndex:2,
        }} />
      </section>

      {/* ╔══════════════════════════════════════════════════════╗
          ║  PHOTO SECTIONS — full-bleed split-screen            ║
          ╚══════════════════════════════════════════════════════╝ */}
      {PHOTOS.map((photo, i) => {
        const isLeft = i % 2 === 0   // text on left, photo on right
        return (
          <section
            key={photo.src}
            ref={el => (sectionRefs.current[i] = el)}
            style={{
              height: "100vh",
              display: "grid",
              gridTemplateColumns: "1fr 1fr",
              overflow: "hidden",
              position: "relative",
              background: SECTION_BG[i],
            }}
          >
            {/* ── TEXT PANEL ── */}
            <div style={{
              order: isLeft ? 0 : 1,
              display: "flex", flexDirection: "column", justifyContent: "center",
              padding: "clamp(2.5rem,6vw,6rem) clamp(2rem,5vw,5rem)",
              position: "relative", zIndex: 2,
              background: SECTION_BG[i],
            }}>
              {/* Eyebrow */}
              <p ref={el => (eyebrowRefs.current[i] = el)} style={{
                fontFamily: "'DM Mono',monospace", fontSize: "0.58rem",
                color: P.terracotta, letterSpacing: "0.4em", textTransform: "uppercase",
                margin: "0 0 1.5rem", opacity: 0, willChange: "transform,opacity",
              }}>
                {photo.idx} / {String(PHOTOS.length).padStart(2,"0")}
              </p>

              {/* Giant scrambling number */}
              <div style={{ overflow: "hidden", lineHeight: 0.85, marginBottom: "2.5rem" }}>
                <span
                  ref={el => (bigNumRefs.current[i] = el)}
                  style={{
                    display: "block",
                    fontFamily: "'DM Mono',monospace",
                    fontSize: "clamp(6rem,16vw,14rem)",
                    fontWeight: 300, color: P.cream,
                    letterSpacing: "-0.05em", lineHeight: 0.85,
                    willChange: "transform,opacity",
                    opacity: 0,
                  }}
                >
                  {photo.idx}
                </span>
              </div>

              {/* Caption with word-masked reveal */}
              <p style={{
                fontFamily: "'Cormorant Galatia',serif",
                fontSize: "clamp(1.2rem,2.4vw,1.75rem)", fontWeight: 300,
                fontStyle: "italic", color: P.sand,
                margin: "0 0 2rem", lineHeight: 1.45,
                overflow: "hidden",
              }}>
                {photo.caption.split(" ").map((word, wi) => (
                  <span key={wi} style={{ display:"inline-block",overflow:"hidden",marginRight:"0.28em",verticalAlign:"bottom" }}>
                    <span ref={el => {
                      if (!wordRefs.current[i]) wordRefs.current[i] = []
                      wordRefs.current[i][wi] = el
                    }} style={{ display:"inline-block",willChange:"transform,opacity",opacity:0 }}>
                      {word}
                    </span>
                  </span>
                ))}
              </p>

              {/* Terracotta sweep rule */}
              <div ref={el => (sweepRefs.current[i] = el)} style={{
                height: "1px", width: "100px",
                background: `linear-gradient(to right,${P.terracotta},rgba(193,103,58,0))`,
                transformOrigin: "left center", transform: "scaleX(0)",
                willChange: "transform",
              }} />

              {/* Subtle corner accent — bottom right of text panel */}
              <div aria-hidden="true" style={{
                position:"absolute",bottom:"2rem",right:"2rem",
                width:"20px",height:"20px",
                borderBottom:`1px solid rgba(193,103,58,0.3)`,
                borderRight:`1px solid rgba(193,103,58,0.3)`,
                pointerEvents:"none",
              }} />
            </div>

            {/* ── PHOTO PANEL ── */}
            <div
              ref={el => (imgPanelRefs.current[i] = el)}
              style={{
                order: isLeft ? 1 : 0,
                position: "relative", overflow: "hidden",
                willChange: "clip-path",
              }}
            >
              <img
                ref={el => (imgRefs.current[i] = el)}
                src={photo.src} alt={photo.caption} onError={handleImgError}
                style={{
                  position: "absolute", inset: 0,
                  width: "100%", height: "120%", top: "-10%",
                  objectFit: "cover", display: "block", willChange: "transform",
                }}
              />

              {/* Placeholder */}
              <div data-placeholder="true" style={{
                display:"none",position:"absolute",inset:0,
                background:`linear-gradient(${PH_GRADIENTS[i]})`,
                alignItems:"center",justifyContent:"center",flexDirection:"column",gap:"0.85rem",
              }}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                  stroke="rgba(245,239,230,0.35)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                  <rect x="3" y="3" width="18" height="18" rx="2" />
                  <circle cx="8.5" cy="8.5" r="1.5" />
                  <polyline points="21 15 16 10 5 21" />
                </svg>
                <span style={{ fontFamily:"'DM Mono',monospace",color:"rgba(245,239,230,0.25)",fontSize:"0.6rem",letterSpacing:"0.3em" }}>{photo.idx}</span>
              </div>

              {/* Scanline overlay — techy CRT feel */}
              <div aria-hidden="true" style={{
                position:"absolute",inset:0,
                backgroundImage:"repeating-linear-gradient(0deg,transparent,transparent 3px,rgba(0,0,0,0.04) 3px,rgba(0,0,0,0.04) 4px)",
                pointerEvents:"none",zIndex:2,
              }} />

              {/* Side gradient bleed into text panel */}
              <div aria-hidden="true" style={{
                position:"absolute",inset:0,
                background: isLeft
                  ? "linear-gradient(to left,transparent 70%,rgba(0,0,0,0.25) 100%)"
                  : "linear-gradient(to right,transparent 70%,rgba(0,0,0,0.25) 100%)",
                pointerEvents:"none",zIndex:3,
              }} />

              {/* Top-left targeting bracket */}
              <div aria-hidden="true" style={{
                position:"absolute",top:"1.5rem",left:"1.5rem",
                width:"22px",height:"22px",
                borderTop:`1px solid rgba(193,103,58,0.45)`,
                borderLeft:`1px solid rgba(193,103,58,0.45)`,
                pointerEvents:"none",zIndex:4,
              }} />
              {/* Bottom-right targeting bracket */}
              <div aria-hidden="true" style={{
                position:"absolute",bottom:"1.5rem",right:"1.5rem",
                width:"22px",height:"22px",
                borderBottom:`1px solid rgba(193,103,58,0.45)`,
                borderRight:`1px solid rgba(193,103,58,0.45)`,
                pointerEvents:"none",zIndex:4,
              }} />

              {/* Photo index watermark */}
              <div aria-hidden="true" style={{
                position:"absolute",bottom:"2rem",
                [isLeft?"right":"left"]:"2rem",
                fontFamily:"'DM Mono',monospace",fontSize:"0.55rem",
                color:"rgba(245,239,230,0.2)",letterSpacing:"0.2em",
                pointerEvents:"none",zIndex:4,
              }}>
                {photo.idx} / {String(PHOTOS.length).padStart(2,"0")}
              </div>
            </div>

            {/* Thin top border */}
            <div aria-hidden="true" style={{
              position:"absolute",top:0,left:0,right:0,height:"1px",
              background:"rgba(217,197,160,0.05)",zIndex:5,
            }} />
          </section>
        )
      })}

      {/* ╔══════════════════════════════════════════════════════╗
          ║  FOOTER                                              ║
          ╚══════════════════════════════════════════════════════╝ */}
      <footer style={{
        minHeight:"36vh",display:"flex",flexDirection:"column",
        alignItems:"center",justifyContent:"center",gap:"1.25rem",
        background:P.dark,borderTop:"1px solid rgba(217,197,160,0.06)",
      }}>
        <div style={{ width:"1px",height:"44px",background:`linear-gradient(to bottom,${P.terracotta},transparent)`,opacity:0.4 }} />
        <p style={{
          fontFamily:"'DM Mono',monospace",color:P.sand,fontSize:"0.62rem",
          letterSpacing:"0.45em",textTransform:"uppercase",opacity:0.2,margin:0,
        }}>made with love · 2024</p>
      </footer>
    </div>
  )
}
